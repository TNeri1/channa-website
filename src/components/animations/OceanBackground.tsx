...
const OceanBackground = () => {
  ...
  useEffect(() => {
    ...
    // Performance optimization: Use RequestAnimationFrame timestamp
    // instead of incrementing time manually
    const animate = (timestamp: number) => {
      // Convert timestamp to seconds and slow it down
      const timeInSeconds = timestamp / 1000;
      time = timeInSeconds * 0.05; // Control animation speed
      ...
      for (let i = 0; i < waves.count; i++) {
        ...
        // Performance optimization: Adjust steps based on screen width
        const step = Math.max(5, Math.ceil(canvas.width / 200)); // Fewer points for better performance
        
        // Draw wave path with smoother curves
        for (let x = 0; x < canvas.width; x += step) {
          // Combine sine and cosine waves with different frequencies
          const sinComponent = Math.sin(x * waveLength + time + phase) * amplitude;
          const cosComponent = Math.cos(x * waveLength * 0.7 + time * 0.8 + phase/2) * amplitude * 0.4;
